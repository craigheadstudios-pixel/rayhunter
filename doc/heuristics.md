...
It combines up to three signals, none of which were previously visible anywhere in Rayhunter:

* An unusually strong *and* suspiciously stable signal from the serving cell (from the modem's raw LTE signal measurements). A real macro cell's signal strength naturally varies as you move and as the radio environment changes; a rogue device sitting close to its target usually produces a stronger, flatter signal than that.
...
* When a GPS fix is available (see [GPS](configuration.md#gps)) and the serving cell can be matched against Rayhunter's bundled [OpenCellID](https://opencellid.org/) snapshot: your position being far outside that cell's registered location and estimated coverage radius.

The first two have mundane explanations on their own: standing very close to a legitimate tower can look like the first, and genuinely rural or low-density coverage can look like the second. Because of that, each one alone is reported at low severity, and only escalated when combined with each other or with a GPS mismatch for the same cell, following the same "combine weak signals" approach used elsewhere in this document (see **IMSI Requested** and **Incomplete SIB** above). The GPS/OpenCellID mismatch is reported at higher severity on its own, since a real mismatch against a real database entry is a stronger signal than either of the other two — but OpenCellID data itself can be stale or wrong for a given cell, so it's a lead to investigate, not proof by itself. A cell that's simply *absent* from the bundled snapshot is never used as a standalone signal (OpenCellID's coverage is too incomplete for that to mean much), only to nudge up the severity of the other two signals when they're already firing.

The bundled OpenCellID data is a static snapshot baked in at build time for a specific region, not something downloaded or updated automatically — see the top of `lib/src/analysis/opencellid.rs` for how to rescope it to a different area.

One more caveat worth knowing about: the signal measurements identify a cell only by its Physical Cell ID (PCI), which is locally unique, not globally unique. This analyzer correlates PCI with the cell's true identity (from SIB1 broadcasts, which is also what's used for the OpenCellID lookup) by assuming the most recently seen SIB1 belongs to whichever cell is currently "serving" — true the vast majority of the time, but briefly incorrect during a handover between towers.
